...
description: "MEG-XL adapts to new subjects with less labelled data and offers a strong baseline for the 2026 PNPL competition’s broad track."
authors: ["Dulhan Jayalath", "Oiwi Parker Jones"]
tags: ["speech-decoding", "meg", "pre-training", "MEG-XL", "libribrain", "broad-track"]
externalUrl: "https://neural-processing-lab.github.io/blog/brain-to-text-decoding-with-meg-xl/"
externalSource: "Neural Processing Lab"
...
citations:
  - id: mantegna2026competition
    title: >-
      The 2026 PNPL Competition: Word Classification and Efficient Cross-Subject
    ...
    bibtex: |-
      @article{mantegna2026competition,
        title={The 2026 PNPL Competition: Word Classification and Efficient Cross-Subject Generalisation in LibriBrain100},
        author={Mantegna, Francesco and Elvers, Gereon and Jayalath, Dulhan and Landau, Gilad and Kim, Tasha and Özdogan, Miran and Kurth, Luisa and Kwon, Teyun and Cho, SungJun and Ballyk, Benjamin and Fung, Alex and Greer, Anna and Somaiya, Pratik and Herff, Christian and Mantilla Ramos, Yorguin and Abdelhedi, Hamza and Jerbi, Karim and Farquhar, Greg and Shillingford, Brendan and Woolrich, Mark and Parker Jones, Oiwi},
  ...
  - id: ozdogan2025libribrain
    title: >-
      LibriBrain: Over 50 Hours of Within-Subject MEG to Improve Speech Decoding
      Methods at Scale
    authors:
      - Miran Özdogan
      - Gilad Landau
  ...
  - id: dascoli2025decoding
    ...
    year: 2025
    url: 'https://pubmed.ncbi.nlm.nih.gov/41298362/'
    volume: '16'
    pages: '10521'
    doi: 10.1038/s41467-025-65499-0
    bibtex: |-
      @article{dascoli2025decoding,
        title={Towards decoding individual words from non-invasive brain recordings},
        author={d’Ascoli, Stéphane and Bel, Corentin and Rapin, Jérémy and Banville, Hubert and Benchetrit, Yohann and Pallier, Christophe and King, Jean-Rémi},
        journal={Nature Communications},
        year={2025},
        url={https://pubmed.ncbi.nlm.nih.gov/41298362/},
        volume={16},
...
As an analogy, Ilya Sutskever gave the [example](https://youtu.be/GI4Tpi48DlA?t=1125) that an LLM could use the context of a murder mystery book to complete the last line “and the killer was…” In practice, to infer the murderer accurately requires, implicitly or otherwise, understanding the contents of the book, and reasoning about the clues in the book. Our hypothesis was that brain models generally face a similar issue as they are trained with comparatively very little neural context.

## Modelling long-context brain data with MEG-XL

To introduce long-context brain modelling, our 2026 ICML paper designed a model called MEG-XL ([Jayalath et al. 2026](https://arxiv.org/abs/2602.02494)) to learn from long stretches of neural activity. In pre-training, MEG-XL makes productive use of samples that are up to 2.5 minutes long, which is 5 to 300 times longer than the contexts used in past pre-trained brain models. To achieve this, MEG-XL uses a common pre-training task known as “masking”, which predicts parts of the recorded brain activity that are hidden from the model using the surrounding signal. This forces the model to learn to use the available context to “fill in” different parts of the brain signal that are hidden. MEG-XL was pre-trained on over 300 hours of MEG from nearly 900 different subjects to encourage the model to learn how to generalise across various contexts, people, and even scanners and datasets. Once pre-trained, we adapted the model for the downstream task of brain-to-text decoding by fine-tuning it with labelled recordings from a new subject (following [d’Ascoli et al. 2025](https://www.nature.com/articles/s41467-025-65499-0)).

![MEG-XL overview: masked pre-training and word-decoding fine-tuning, 150-second input contexts, and top-10 accuracy compared with prior models across three datasets.](/blog/brain-to-text-decoding-with-meg-xl/meg-xl-overview.png)
...
1. **Pre-training is most helpful when labelled subject-specific fine-tuning data is scarce.** This is exactly what we want, as it is the regime most relevant to reducing the data collection burden for new users.
1. **With enough single-subject data, supervised training remains the gold standard.** MEG-XL is therefore most valuable when collecting large amounts of new labelled data is impractical or even impossible.
1. **Longer pre-training contexts improve accuracy on brain-to-text.** Increasing the amount of continuous neural activity used during pre-training improves the representations learned by the model.
1. **Models have to learn how to use long contexts.** Giving a model more context after it has been trained on short windows does not lead to the same effect. Crucially, the long context must already be present during pre-training.
1. **Long-context models appear to use distant information selectively.** Consistent with what we know about attention, MEG-XL learns when and how to use information from far back in time rather than treating everything in the preceding minutes as equally useful.
